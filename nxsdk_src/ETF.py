...
def setupNetwork(net, numECores, numICores, tau, spikeGen, Enoise, Inoise, Jee):
    ...
    connProtoRecurrent = nx.ConnectionPrototype(weight=Jee, delay=0) 
    ...
    synaptic_connectivity4 = 0.25
    synaptic_matrix4 = np.random.rand(numECores, numICores) < synaptic_connectivity4
    cg2.connect(cg1, prototype=connProto2, connectionMask=synaptic_matrix4)
...
if __name__ == "__main__":        
    ...
    df.to_excel('data/ETF_weights.xlsx', index=False)
    
    # Save the figure
    if haveDisplay:
        plt.show()
    else:
        fileName = "figures/ETF/ETF_weights.png"
